...
const getIngestFiles = ({ ingestFolder, ingestName }) =>
  R.compose(
    R.concat([
      { Key: `${ingestFolder}${ingestName}/manifest.json` },
      { Key: `${ingestFolder}${ingestName}` }
    ]),
    R.filter(
      R.compose(
...
class Times {
  ...
  setNeoStart() {
    this.neoStart = moment(new Date());
  }
  getNeoStart() {
    return this.neoStart;
  }
  setNeoEnd() {
    this.neoEnd = moment(new Date());
  }
  getNeoEnd() {
    return this.neoEnd;
  }
  ...
  setElasticStart() {
    this.elasticStart = moment(new Date());
  }
  getElasticStart() {
    return this.elasticStart;
  }
  setElasticEnd() {
    this.elasticEnd = moment(new Date());
  }
  getElasticEnd() {
    return this.elasticEnd;
  }
